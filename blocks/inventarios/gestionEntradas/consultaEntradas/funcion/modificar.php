...
class RegistradorOrden {
	...
	function procesarFormulario() {
		...
		foreach ( $arreglo as $key => $values ) {
			...
			if ($modificar == false) {
				\inventarios\gestionEntradas\consultaEntradas\funcion\redireccion::redireccionar ( 'noInserto', $_REQUEST ['usuario'] );
			}
		}
		
		if ($modificar != false) {
			
			\inventarios\gestionEntradas\consultaEntradas\funcion\redireccion::redireccionar ( 'inserto', $_REQUEST ['usuario'] );
			exit ();
		} else {
			\inventarios\gestionEntradas\consultaEntradas\funcion\redireccion::redireccionar ( 'noInserto', $_REQUEST ['usuario'] );
			exit ();
